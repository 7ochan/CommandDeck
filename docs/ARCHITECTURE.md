...
On the reference zsh profile, a dedicated prompt-presentation script replaces the user's visible primary prompt inside CommandDeck with a compact two-line prompt containing only the abbreviated current directory and a `❯` command indicator. The existing OSC prompt markers still surround the prompt and remain the command-capture boundary. After a detected command completes, the shell integration emits one presentation-only newline before the next prompt. This spacing is ordinary terminal output.

A browser-only command-section presentation coordinator consumes the existing ordered `command.started` and `command.completed` lifecycle events. The active command remains undecorated and visually connected to its live output. On completion, after previously queued PTY output has been parsed by xterm.js, the coordinator anchors one text-free separator to an xterm buffer marker. The separator is rendered through xterm's decoration API, is hidden from assistive technology and pointer interaction, and never enters the terminal buffer, selection, persisted History, or ANSI stream. Markers follow normal-buffer scrollback and dispose their separators when xterm trims the associated line; reset and session replacement dispose all remaining presentation state.

xterm.js remains the only renderer. The coordinator does not parse terminal bytes, alter input, copy, resize, cursor, or execution behavior, and it creates presentation state per completed command rather than per output line. Alternate-screen applications retain direct PTY behavior and xterm suppresses normal-buffer decorations while the alternate buffer is active. If a completion cannot be decorated, the shell-emitted spacing remains the compatibility fallback. This boundary can later host other lightweight, xterm-anchored section presentation without introducing a React terminal renderer or duplicating Command History.
